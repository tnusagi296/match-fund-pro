...
import { getGraphFounderById } from "@/lib/graph/feed.functions";
import { rankGraphFounders } from "@/lib/graph/thesis-fit";
import type { GraphFounderCard as GraphFounderCardData } from "@/lib/graph/types";
import { useThesis } from "@/lib/thesis";

type FounderRouteData =
  { kind: "demo"; founder: Founder } | { kind: "graph"; founder: GraphFounderCardData };

export const Route = createFileRoute("/founder/$id")({
  loader: async ({ params }): Promise<FounderRouteData> => {
    const demoFounder = founderById(params.id);
    if (demoFounder) return { kind: "demo", founder: demoFounder };

    const graphFounder = await getGraphFounderById({ data: { profileId: params.id } });
    if (!graphFounder) throw notFound();
    return { kind: "graph", founder: graphFounder };
  },
  head: ({ loaderData }) => ({
    meta: [
      {
...
function FounderProfile() {
  const data = Route.useLoaderData() as FounderRouteData;
  return data.kind === "graph" ? (
    <GraphFounderProfile founder={data.founder} />
  ) : (
    <DemoFounderProfile founder={data.founder} />
  );
}

function GraphFounderProfile({ founder }: { founder: GraphFounderCardData }) {
  const [thesis, , hydrated] = useThesis();
  const ranked = useMemo(
    () => (thesis ? rankGraphFounders([founder], thesis)[0] : null),
    [founder, thesis],
  );
    ...
    <AppShell>
      ...
      {!hydrated ? (
        ...
        <div className="mx-auto max-w-3xl">
          <FounderCard
            founder={ranked.founder}
            thesisFit={ranked.thesisFit}
            discoveryPriority={ranked.discoveryPriority}
          />
        </div>
      ) : (
        <div className="mx-auto max-w-xl rounded-2xl glass p-8 text-center">
          <h1 className="font-display text-2xl font-semibold">{founder.name}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Save an investor thesis to calculate evidence-backed Thesis Fit and open Why matched.
          </p>
          <Link
            to="/onboard"
            className="mt-5 inline-flex rounded-full bg-mint px-4 py-2 text-sm font-medium text-primary-foreground"
          >
            Create thesis
          </Link>
        </div>
...
function DemoFounderProfile({ founder }: { founder: Founder }) {
      ...
      <aside className="col-span-3 space-y-4">
        ...
        <div className="rounded-2xl glass p-5">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            <span className="tabular text-2xl font-semibold text-mint">{evidenceConfidence}</span>
        ...
        <div className="rounded-2xl border border-mint/25 bg-mint-soft/25 p-5">
          ...
          <p className="mt-2 text-xs leading-relaxed text-foreground/85">
            30-minute intro call to validate{" "}
            {founder.fitBreakdown.market < 80 ? "market thesis" : "execution plan"}.
          </p>
...
function EvidenceCard({ source }: { source: ReturnType<typeof discoveredSources>[number] }) {
  const Icon =
    source.kind === "github"
      ? Github
  ...
  return (
    <div className="grid grid-cols-12 gap-5">
      <div className="col-span-8 space-y-4">
        <section className="rounded-2xl glass p-5">
          <SectionHead icon={Github} title="GitHub metrics" note="Publicly observed" />
        ...
        <div className="sticky top-56 rounded-2xl glass p-5">
          ...
          <div className="mt-3 space-y-2 text-xs">
            ...
            <SummaryRow
              ...
              tone="mint"
            />
            <SummaryRow label="Team completeness" value={founder.signals.teamSize} tone="neutral" />
          </div>
        </div>
      </aside>
    </div>
...
function IdeaBlock({ label, body, status }: { label: string; body: string; status: string }) {
  return (
    <section className="rounded-2xl glass p-5">
      <div className="flex items-center justify-between">
        <div className="text-[11px] font-medium uppercase tracking-wider text-mint">{label}</div>
        <span className="rounded-full border border-border/60 bg-background/40 px-2 py-0.5 text-[10px] text-muted-foreground">
          {status}
        </span>
      </div>
