  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10 space-y-6">
      <header className="space-y-2">
        <div className="text-xs uppercase tracking-widest text-white/50">Stage 1 — GitHub-first</div>
        <h1 className="text-3xl font-semibold tracking-tight">Discover public builders</h1>
        <p className="text-sm text-white/60 max-w-2xl">
          Search GitHub by topic/stars/activity, then ingest each repo owner into the evidence
          graph as a <em>Potential builder — founder status unverified</em>. Ingested profiles are
          published to the Discover feed with real GitHub evidence attached.
        </p>
      </header>

      <div className="glass rounded-2xl p-5 space-y-4">
        <label className="block text-xs uppercase tracking-widest text-white/50">GitHub search query</label>
        <input
          value={query}
        ...
        <div className="flex items-center gap-4">
          <label className="text-xs text-white/60">Candidates
            <input
              type="number"
              min={1}
              max={10}
      ...
      {result && (
        <div className="space-y-4">
          <div className="glass rounded-2xl p-5 flex flex-wrap gap-6 text-sm">
            ...
            <Stat label="Failed" value={result.failed} tone={result.failed > 0 ? "warn" : undefined} />
            <Stat
              label="GitHub auth"
                  ...
                  {c.topRepo.topics.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      {c.topRepo.topics.slice(0, 6).map((t) => (
                        <span key={t} className="text-[10px] rounded bg-white/5 border border-white/10 px-2 py-0.5 text-white/60">
                          {t}
                        </span>
                      ))}
                    </div>
