...
import {
  createOrUpdateFounderProfile,
  persistGraphIngestion,
...
export async function searchRepositories(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `GitHub repo search failed [${response.status}]: ${body.slice(0, 200)}`,
    );
  }
...
export async function runGitHubDiscovery(options: {
  ...
  for (const repo of picked) {
    ...
    try {
      ...
      // Publish so the Discover feed can surface it.
